...
let demo () =
  ...
  let check_layout =
    L.tower ~margins:0 [ check_title; check1; check2; check3 ]
  in
  ...
  let radio =
    Radiolist.vertical
      [|
        "select this"; "or rather that"; "maybe this"; "worst case, this one";
      |]
  in
  let radio_layout =
    L.tower ~margins:0 ~sep:0 [ radio_title; Radiolist.layout radio ]
  in
  ...
  let text_head =
    W.rich_text ~size:20 ~w:width ~h:30
      Text_display.(page [ bold (para fable_title) ])
  in
  let text = W.text_display ~w:width ~h:630 fable in
  let text_ref =
    W.rich_text ~w:width ~h:20 Text_display.(page [ italic (para fable_ref) ])
  in
  let text_layout =
    L.tower
  ...
  let image = W.image ~w:(width / 2) "images/play.png" in
  ...
  let bottom =
    L.flat ~align:Draw.Max ~margins:0
      [ image_layout; Space.hfill (); quit_layout ]
  in
  ...
  let tabs =
    Tabs.create ~slide:Avar.Right [ ("Page 1", page1); ("More stuff", page2) ]
  in
